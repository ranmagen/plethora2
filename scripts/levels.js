...
$(function () {
    ...
    ctx.canvas.width = window.innerWidth;//$("#sidebar").offset().left;
    ctx.canvas.height = window.innerHeight - 60;//$("#header").height();
    ...
    dbRef.once("value", function (data) {
        ...
        for (var i = 0; i < levelsCnt; i++) {
           // var levelName = data.child(i).val().name;
            ...
            $('<div><img class="level-img" src="images/levels/level' + lvl + '.png"/><p>' + data.child(i).val().name + '</p></div>').attr('id',
